...
| **system_prompt** | Write anything you want to customize the bot's behavior!<br /><br />**Leave blank for no system prompt.**<br /><br />**You can use the `{date}` and `{time}` tags in your system prompt to insert the current date and time, based on your host computer's time zone.** <br /><br />For long system prompts (for roleplaying or other use cases), you can set up `system_prompt.txt` at `llmcord/system_prompt.txt` instead in `config.yaml`. |
